...
def configure_globals():
    ...
    global val_data_dir
    global img_train_dir
    global noisy_train_dir
    global debug_train_dir
    global img_val_dir
    global noisy_val_dir
    global debug_val_dir
    global words_list
    global patches_list
    global logo_list
    global image_blocks
...
def __process(index):
    ...
    try:
        ...
        # Put stuff on the img and return the mask.
...
def get_patches():
    ...
    # Resolution widths
    # 128*20 is the base
    # resolutions = [128 * 20, 128 * 19.5, 128 * 19, 128 * 18, 128 * 18.5, 128 * 17.0] # unused?

    resolutions = [128 * 16] # 2048
...
def print_lines_aligned(pil_img, boxes):
    ...
    def make_txt():
        ...
        if np.random.choice([0, 1], p=[0.8, 0.2]):
            ...
            txt = ''.join(random.choice(digits) for i in range(c))
    ...
    # Random coordinate on the image cut into 12 blocks long and 16 blocks high.
    ...
    if np.random.choice([True, False], p=[0.5, 0.5]):
        ...
        # augment the logo if needed (randomly?)
        ...
        if np.random.choice([True, False], p=[0.5, 0.5]):
            ...
            img = img.convert('RGB')
            # randomize the size of the qr code
    ...
    while True:
        ...
        # Should this be renamed to max_height?
